...
class YeziPlatform:
    ...
    def get_phone_number(self, special=None, phone_num=None, scope=None):
        ...
        params = {
            ...
            #'scope': "185,171,153"
            #'scope': "130"
            'scope_black':"192,188,158,152,167,165,162"
            #'scope_black':"192,188,158,152,167,165,162,181,180,177,189,133,195,159,187"
            #含有积分号段 177,151,173,132,199,189,131,133,191,190,195,150,134,171,181,185,178,136,137,183,187,130,135,153,156,166,182,175,197,198,180,193,155,186,159,138
        }
...
def main():
    ...
    filter_phones = [] # 避免已检测的手机号重复处理
    query_integral = 1 # 是否查询积分
    query_repetition = 1 # 是否检测重复号
    query_black = 1 # 是否检测黑号
    query_filter = 1 # 是否检测的手机号重复处理
    ...
    while current_success_count < target_success_count:
        try:
            ...
            # 检测是否是黑号
            if query_black == 1:
                if phone_number in black_phone:
                    logger.info(f"手机号 {phone_number} 已在黑名单")
                    yezi.free_mobile(phone_number)
                    time.sleep(3)
                    continue
            ...
            # 避免已检测的手机号重复处理
            if query_filter == 1:
                if phone_number in filter_phones:
                    logger.info(f"手机号 {phone_number} 已在本次会话中处理过，跳过并重新获取")
                    yezi.free_mobile(phone_number)
                    time.sleep(3)
                    continue
                else:
                    # 将当前手机号添加到过滤列表中
                    filter_phones.append(phone_number)
                    logger.info(f"手机号 {phone_number} 已添加到过滤列表，当前过滤列表长度: {len(filter_phones)}")
